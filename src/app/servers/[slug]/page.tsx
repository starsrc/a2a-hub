    ...
    return (
        <div className="mx-auto max-w-3xl space-y-6 py-8">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                {s.website ? (
                    <a className="rounded-md bg-foreground px-3 py-1.5 text-sm text-background hover:opacity-90"
                       href={s.website} target="_blank" rel="noreferrer">Website</a>
                ) : null}
                {s.repo ? (
                    <a className="rounded-md border px-3 py-1.5 text-sm hover:bg-foreground/5" href={s.repo}
                       target="_blank" rel="noreferrer">GitHub</a>
                ) : null}
            </div>
            <div className="rounded-md border p-4 text-sm text-foreground/80">
                <p>
                    Deployment instructions: Please deploy according to the project documentation. For detailed steps,
                    see
                    {s.website ? (
